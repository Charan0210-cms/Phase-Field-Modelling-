"""
6.3  1D Heat Equation – FDM and Neumann BC – Zero Flux at Both Boundaries (2)
Step function as initial condition and zero flux at both boundaries.
"""
...
# Step function initial condition: T=1 for indices 1000..2000, else 0
for k in range(0, M):
    if 1000 <= k <= 2000:
        T[k, 0] = 1
    else:
...
plt.title('1D Heat Equation – FDM & Neumann BC (Zero Flux, Step IC)')
plt.legend(title='Time step')
plt.tight_layout()
plt.savefig('6_3_neumann_zero_flux_2_2D.png', dpi=150)
plt.show()
print("Saved: 6_3_neumann_zero_flux_2_2D.png")
